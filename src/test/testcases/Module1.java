...
public class Module1 extends UserLibrary
{
	public static void TC1(){
		...
		status=OpenUrl(URL);
		if(status)
			LogEventWithScreeshot("info", "Application is launched");
		else
			LogEventWithScreeshot("fail", "Application is not launched");
		status = SetText("//*[@id='txtUsername']", UserName);
		if(status)
			LogEventWithScreeshot("info", "Username is entered");
		else
			LogEventWithScreeshot("fail", "Username is not entered");
		
		status = SetText("//*[@id='txtPassword']", UserName);
		if(status)
			LogEventWithScreeshot("info", "Password is entered");
		else
			LogEventWithScreeshot("fail", "Password is not entered");
		status = ClickElement("//*[@id='btnLogin']");
		if(status)
			LogEventWithScreeshot("info", "Button is entered");
		else
			LogEventWithScreeshot("fail", "Button is not entered");
